...
export default function InsightsPage() {
  const [releasedInsights, setReleasedInsights] = useState([]);
  ...
  useEffect(() => {
    ...
    if (sorted.length > 0) {
      ...
      setReleasedInsights(sorted.slice(1));
    } else {
      setFeatured(insights[0]);
      setReleasedInsights([]);
    }
  }, []);
  ...
  return (
    <div className="blueprint-grid min-h-screen pt-12 pb-8 relative z-10">
      ...
      <section className="max-w-[1400px] mx-auto px-6 md:px-12 pt-20 overflow-hidden">
        ...
        >
          {releasedInsights.length > 0 ? (
            releasedInsights.map((item) => (
              <div
                key={item.slug}
            ...
            <div className="w-full text-center py-12 border border-dashed border-border/60 text-muted font-body text-sm rounded-none">
              [ Archive is currently empty — previous insights will appear here tomorrow ]
            </div>
          )}
        </div>
      </section>
